...
public abstract class BaseMicroserviceVerticle extends AbstractVerticle {
  ...
  protected ServiceDiscovery discovery; //发现实例
  protected CircuitBreaker circuitBreaker; //　断路由实例
  protected Set<Record> registeredRecords = new ConcurrentHashSet<>(); // 当前已发布的服务记录的集合，用于在结束Verticle时注销服务
   ...
   */
  // 发布服务的通用方法
  private Future<Void> publish(Record record) { // Record代表一个服务
    if (discovery == null) {
      try {
        start();
      } catch (Exception e) {
